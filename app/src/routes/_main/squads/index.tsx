...
export const Route = createFileRoute('/_main/squads/')({
...
export const MOCK_SQUADS = [
  {
    ...
    description: "Building the next gen iOS and Android application for the main product.",
    leader: {
      name: "Sarah Connor",
  ...
  },
  {
    ...
    description: "Revamping the data pipeline and analytics dashboard infrastructure.",
    leader: {
      name: "John Wick",
  ...
  },
  {
    ...
    description: "Redesigning the public facing marketing website for Q4 launch.",
    leader: {
      name: "Tony Stark",
    ...
    <div className="min-h-screen pb-24 bg-transparent animate-in fade-in duration-500">
      <main className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-primary">Squads</h1>
            <p className="text-muted-foreground mt-1">Manage your cross-functional teams and track progress.</p>
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center bg-card/60 backdrop-blur-sm p-4 rounded-xl border border-border shadow-sm">
          ...
          <div className="flex gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 no-scrollbar">
             <Button 
               ...
               className={statusFilter === null ? "bg-background shadow-sm border border-border font-medium" : "text-muted-foreground"}
             >
               All
             </Button>
             <Button 
               ...
               className={statusFilter === "active" ? "bg-background shadow-sm border border-border font-medium text-green-700" : "text-muted-foreground"}
             >
               Active
             </Button>
             <Button 
               ...
               className={statusFilter === "recruiting" ? "bg-background shadow-sm border border-border font-medium text-accent-foreground" : "text-muted-foreground"}
             >
               Recruiting
             </Button>
             <Button 
               ...
               className={statusFilter === "completed" ? "bg-background shadow-sm border border-border font-medium text-blue-700" : "text-muted-foreground"}
             >
               Completed
             </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSquads.map((squad, index) => (
            <Link key={squad.id} to="/squads/$squadId/edit" params={{squadId: squad.id}} className="block h-full"> 
            {/* Note: Edit route might not exist yet, but link structure supports it */}
              <motion.div
              ...
              >
                <Card className="h-full border-none shadow-card hover:shadow-xl transition-all duration-300 bg-card/80 backdrop-blur-sm group cursor-pointer overflow-hidden relative">
                  ...
                  <CardHeader className="pb-3 relative z-10">
                    <div className="flex justify-between items-start mb-2">
                      <Badge 
                        ...
                        className={`
                          ...
                          ${squad.status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : ''}
                          ${squad.status === 'recruiting' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' : ''}
                          ${squad.status === 'completed' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : ''}
                        `}
                      >
                        {squad.status}
                      </Badge>
                    ...
                    <h3 className="font-bold text-lg text-primary group-hover:text-accent-foreground transition-colors line-clamp-1">
                      {squad.name}
                    </h3>
                  ...
                  <CardContent className="pb-3 space-y-4 relative z-10">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {squad.techStack.slice(0, 3).map(tech => (
                        <span key={tech} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-muted text-muted-foreground border border-border/50 group-hover:border-accent/30 transition-colors">
                          {tech}
                        </span>
                      ))}
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="font-bold text-foreground">{squad.progress}%</span>
                      </div>
                      <Progress value={squad.progress} className="h-1.5 bg-muted" indicatorClassName="bg-primary group-hover:bg-accent-foreground transition-colors" />
                    </div>
                  </CardContent>
                  ...
                  <CardFooter className="pt-3 border-t border-border/40 flex justify-between items-center bg-background/40 relative z-10">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-muted-foreground">
                        Lead: <span className="text-foreground">{squad.leader.name.split(" ")[0]}</span>
                      </span>
                    </div>
          ...
          {/* Add New Card (Empty State in Grid) */}
          <Link to="/squads/create" className="block h-full">
            <motion.div 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              whileHover={{ 
            ...
            >
              ...
              <h3 className="font-semibold text-lg text-foreground group-hover:text-accent-foreground mb-1">Create New Squad</h3>
              <p className="text-sm text-muted-foreground max-w-[200px]">Start a new cross-functional team for your next initiative.</p>
            </motion.div>
          </Link>
        </div>
      </main>
